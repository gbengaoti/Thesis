...
electrostatic = np.reshape(electrostatic, [-1, 200, 1])
print (electrostatic.shape)

NUM_EXAMPLES = 2000
test_input = X_data[NUM_EXAMPLES:NUM_EXAMPLES+1000]
test_output = electrostatic[NUM_EXAMPLES:NUM_EXAMPLES+1000] 

valid_input = X_data[NUM_EXAMPLES+1000:4000]
valid_output = electrostatic[NUM_EXAMPLES+1000:4000]

train_input = X_data[:NUM_EXAMPLES]
train_output = electrostatic[:NUM_EXAMPLES]
...
model.add(Bidirectional(GRU(hidden_units, return_sequences=True, kernel_regularizer=regularizers.l2(0.01))))
...
model.fit(np.asarray(train_input), np.asarray(train_output), 
	validation_data=(np.asarray(valid_input), 
		np.asarray(valid_output)), epochs=30, batch_size=batch_size)#, callbacks=[save_loss_callback, 
	#save_valloss_callback])

# Final evaluation of the model
# numpy_loss_history = np.array(loss_history)
# np.savetxt("convergence_first_model8units_train.txt", numpy_loss_history, delimiter=",")

# numpy_vloss_history = np.array(val_loss_history)
# np.savetxt("convergence_first_model8units_valid.txt", numpy_vloss_history, delimiter=",")
...
electostatic_pred = np.array(prediction)
np.savetxt("electrostatic_pred.txt", electostatic_pred, delimiter=",")
prediction = np.reshape(prediction, [-1, 200])
