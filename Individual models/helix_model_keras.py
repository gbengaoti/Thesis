...
helix = np.reshape(helix, [-1, 200, 1])
print (helix.shape)

NUM_EXAMPLES = 2000
test_input = X_data[NUM_EXAMPLES:NUM_EXAMPLES+1000]
test_output = helix[NUM_EXAMPLES:NUM_EXAMPLES+1000] 

valid_input = X_data[NUM_EXAMPLES+1000:4000]
valid_output = helix[NUM_EXAMPLES+1000:4000]

train_input = X_data[:NUM_EXAMPLES]
train_output = helix[:NUM_EXAMPLES]
...
prediction = model.predict(np.asarray(test_input), batch_size=batch_size)
helix_pred = np.array(prediction)
np.savetxt("helix_pred.txt", helix_pred, delimiter=",")
