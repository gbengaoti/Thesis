...
coil = np.reshape(coil, [-1, 200, 1])
print (coil.shape)

NUM_EXAMPLES = 2000
test_input = X_data[NUM_EXAMPLES:NUM_EXAMPLES+1000]
test_output = coil[NUM_EXAMPLES:NUM_EXAMPLES+1000] 

valid_input = X_data[NUM_EXAMPLES+1000:4000]
valid_output = coil[NUM_EXAMPLES+1000:4000]

train_input = X_data[:NUM_EXAMPLES]
train_output = coil[:NUM_EXAMPLES]
...
prediction = model.predict(np.asarray(test_input), batch_size=batch_size)
coil_pred = np.array(prediction)
np.savetxt("coil_pred.txt", coil_pred, delimiter=",")
