...
sheet = np.reshape(sheet, [-1, 200, 1])
print (sheet.shape)

NUM_EXAMPLES = 2000
test_input = X_data[NUM_EXAMPLES:NUM_EXAMPLES+1000]
test_output = sheet[NUM_EXAMPLES:NUM_EXAMPLES+1000] 

valid_input = X_data[NUM_EXAMPLES+1000:4000]
valid_output = sheet[NUM_EXAMPLES+1000:4000]

train_input = X_data[:NUM_EXAMPLES]
train_output = sheet[:NUM_EXAMPLES]
...
model.fit(np.asarray(train_input), np.asarray(train_output), 
	validation_data=(np.asarray(valid_input), 
		np.asarray(valid_output)), epochs=20, batch_size=batch_size)#, callbacks=[save_loss_callback, 
...
# print('MSE error:', mean_squared_error(test_output, prediction))
# acc =  r2_score(test_output, prediction)
# print('Accuracy:', (acc))
print ("EVALUATION OF THE TEST SET")
prediction = model.predict(np.asarray(test_input), batch_size=batch_size)
sheet_pred = np.array(prediction)
np.savetxt("sheet_pred.txt", sheet_pred, delimiter=",")
prediction = np.reshape(prediction, [-1, 200])
test_output = np.reshape(test_output, [-1, 200])
acc =  r2_score(test_output, prediction)
print('R2 score:', (acc))
