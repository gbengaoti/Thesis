...
def build_rnn(init):
    ...
    l_forward_back = lasagne.layers.GRULayer(
            l_in, N_HIDDEN,
            only_return_final=False, backwards=True)

    l_concat1 = lasagne.layers.ElemwiseSumLayer([l_forward_1,l_forward_back])

    l_forward_2 = lasagne.layers.GRULayer(
            l_concat1, N_HIDDEN,
            only_return_final=False)

    l_forward_back2 = lasagne.layers.GRULayer(
            l_concat1, N_HIDDEN,
            only_return_final=False, backwards=True)

    l_concat2 = lasagne.layers.ElemwiseSumLayer([l_forward_2,l_forward_back2])

    l_forward_3 = lasagne.layers.GRULayer(
            l_concat2, N_HIDDEN,
            only_return_final=False)

    l_forward_back3 = lasagne.layers.GRULayer(
            l_concat2, N_HIDDEN,
            only_return_final=False, backwards=True)

    l_concat3 = lasagne.layers.ElemwiseSumLayer([l_forward_3,l_forward_back3])

    l_forward_4 = lasagne.layers.GRULayer(
            l_concat3, N_HIDDEN,
            only_return_final=False)

    l_forward_back4 = lasagne.layers.GRULayer(
            l_concat3, N_HIDDEN,
            only_return_final=False, backwards=True)

    l_concat4 = lasagne.layers.ElemwiseSumLayer([l_forward_4,l_forward_back4])

    l_forward_5 = lasagne.layers.GRULayer(
            l_concat4, N_HIDDEN,
            only_return_final=False)

    l_forward_back5 = lasagne.layers.GRULayer(
            l_concat4, N_HIDDEN,
            only_return_final=False, backwards=True)

    l_concat5 = lasagne.layers.ElemwiseSumLayer([l_forward_5,l_forward_back5])

    l_forward_6 = lasagne.layers.GRULayer(
            l_concat5, N_HIDDEN,
            only_return_final=False)

    l_forward_back6 = lasagne.layers.GRULayer(
            l_concat5, N_HIDDEN,
            only_return_final=False, backwards=True)

    l_concat6 = lasagne.layers.ElemwiseSumLayer([l_forward_6,l_forward_back6])

    l_forward_7 = lasagne.layers.GRULayer(
            l_concat6, N_HIDDEN,
            only_return_final=False)

    l_forward_back7 = lasagne.layers.GRULayer(
            l_concat6, N_HIDDEN,
            only_return_final=False, backwards=True)

    l_concat7 = lasagne.layers.ElemwiseSumLayer([l_forward_7,l_forward_back7])

    l_shp = lasagne.layers.ReshapeLayer(l_concat7, (-1, N_HIDDEN))
...
with rnn:
    ...
    v_params = pm.variational.advi_minibatch(
        ...
        total_size=total_size, learning_rate=0.001, epsilon=1e-8)
